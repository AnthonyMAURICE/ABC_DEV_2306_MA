...
public class App {
	...
	public static void main(String[] args) {
		...
		//booléens de test
		String type;
		boolean test1, test2, test3, test4, test5, test6, test7, test8, test9, test10, test11, test12, testLeft, testRight, testMixer, testMixer2;
		int posX, posY;
				
		Robot cylon = new Robot("Cylon", "WarBot", 200, true, true, false, 10, 0); //objet Robot construit avec paramètres
		cylon.setPower();
		test1 = cylon.setMouvement(Mouvement.AVANT); //renvoit true car le cylon est activé 
		cylon.setMouvement(Mouvement.ARRIERE);
		test2 = cylon.setMouvement(Mouvement.GAUCHE);
		testLeft = cylon.setMouvement(Mouvement.AVANT); // test si le cylon avance bien sur le bon axe suite à sa rotation à gauche
		...
		r2d2.setPower();
		test11 = r2d2.setMouvement(Mouvement.DETRUIRE); // renvoit true car le mouvement est valide, mais pas le bon type de robot pour la fonction associée
